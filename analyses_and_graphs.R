...
load("vultures-pas.RData")
save(file = "vultures-pas.RData", invupa, invunpa, rhvupa, rhvunpa, wrvupa, wrvunpa, 
     sbvupa, sbvunpa, higrpa, higrnpa, egvupa, egvunpa)
datap = data %>% filter(!is.na(NAME))
datas = data %>% filter(is.na(NAME))
period = "long"
scol = "#869B27"

trendsa = freqtrends(data=datap, species="Egyptian Vulture", error=T, type="prot", nsim=1000)
trendsb = freqtrends(data=datas, species="Egyptian Vulture", error=T, type="full", nsim=1000)
trends1a = stdtrends(trendsa)
trends1b = stdtrends(trendsb)
trends1a$type = "PAs"
trends1b$type = "Outside PAs"
trends1 = rbind(trends1a,trends1b)
...
period = "long"
scol = "#869B27"
trends = freqtrends(data=datat, species="Great Hornbill", error=T, type="null", nsim=1000)
...
if (any(ybreaks != 100))
{
  ...
  if (min(tmp1) < 0 & any(tmp1 == 100))
  {
    ...
    limu = limu + round(0.03*(limu-liml))
  }
  if (any(tmp2 == 100))
  {
    ybreaks = tmp2
...
scol = c("#869B27","#E49B36")

ggp = ggplot(temp, aes(x=timegroups, y=nmfreqbyspec, col = type, fill = type)) + 
  geom_point(size = 3) +
  geom_line(size = 1.5) +
  geom_hline(yintercept = ybreaks[1], linetype = "dotted", size = 0.7) +
  geom_hline(yintercept = ybreaks[2], linetype = "dotted", size = 0.7) +
  ...
  geom_hline(yintercept = ybreaks[5], linetype = "dotted", size = 0.7) +
  geom_ribbon(aes(x = timegroups, ymin = minci,
                  ymax = maxci), colour = NA, alpha = 0.3) +
  xlab("years") +
  ylab("change in abundance index")
...
ggpx1 = ggpx +
  {if(period == "long")labs(tag = paste(sprintf('\u25A0')," ",sprintf('\u25A0'),
                                        " ",sprintf('\u25A0')," ",sprintf('\u25A0')))}+
  {if(period == "long")theme(plot.tag.position = c(0.84, 0.15),
                             plot.tag = element_text(size = 30, colour = 'white', face = 'bold'))}


sps = as.character(unique(temp$species))
name = paste(sps,"_","trend.jpeg",sep="")
